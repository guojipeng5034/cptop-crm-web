<template>
  <h3 class="text-center m-0">个人排名</h3>
  ...
  <el-card v-for="item in list.rankData.data.multiple.personal" :key="item.id" class="h-10 mb-2 ranking" shadow="always">
    <div class="flex justify-between items-center">
      <div class="flex items-center">
        <Icon :icon="item.icon" :size="20" />
        <span class="ml-12">{{ item.name }}</span>
        <span class="ml-12">{{ item.group }}</span>
      </div>
...
list.rankData.data.multiple.personal.find((item => item.id === 1)).icon = 'noto:1st-place-medal'
list.rankData.data.multiple.personal.find((item => item.id === 2)).icon = 'noto:2nd-place-medal'
list.rankData.data.multiple.personal.find((item => item.id === 3)).icon = 'noto:3rd-place-medal'
list.rankData.data.multiple.personal
