<template>
  <Dialog title="批量添加停课券" v-model="dialogVisible" width="450">
    ...
    <el-form ref="formRef" :model="formData" :rules="formRules" label-width="90" v-loading="formLoading">
      <el-form-item label="停课券" prop="couponId">
        <el-select v-model="formData.couponId" placeholder="请选择或检索停课券" filterable clearable>
...
import { getIntDictOptions, DICT_TYPE, getSuspendOptions } from '@/utils/dict'
import { CustomerSuspendApi } from '@/api/crm/sales/suspend/customerSuspend'
import { getAccessToken, getTenantId } from '@/utils/auth'
import download, { exportExcel } from '@/utils/download'

/** 批量添加停课券 表单 */
defineOptions({ name: 'BatchImportSuspendForm' })
...
let importUrl =
  ...
  '/crm/sales/customerSuspend/import' // 上传地址
...
const formRules = reactive({
  couponId: [{ required: true, message: '停课券为必选项', trigger: 'change' }],
...
const submitFormSuccess = (response: any) => {
  ...
  ElMessageBox.confirm(text, '导入结果', {
    ...
    cancelButtonText: '批量添加停课券失败学员文件链接',
    ...
    .then(() => { })
    .catch((e) => {
      exportExcel(['学员'], response.data.failureResult, '批量添加停课券失败学员ID.xlsx')
...
const downloadTemplate = async () => {
  const res = await CustomerSuspendApi.getCustomerSuspendTemplate()
  download.excel(res, '停课券批量添加模版.xlsx')
}
</script>
