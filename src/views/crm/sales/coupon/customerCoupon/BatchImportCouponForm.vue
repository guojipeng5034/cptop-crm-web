<template>
  <Dialog title="批量添加优惠券" v-model="dialogVisible" width="450">
...
import { getIntDictOptions, DICT_TYPE, getCouponOptions } from '@/utils/dict'
import { CustomerCouponApi } from '@/api/crm/sales/coupon/customerCoupon'
import { getAccessToken, getTenantId } from '@/utils/auth'
import download, { exportExcel } from '@/utils/download'

/** 批量添加优惠券 表单 */
defineOptions({ name: 'BatchImportCouponForm' })
...
let importUrl =
  ...
  '/crm/sales/customerCoupon/import' // 上传地址
...
const submitFormSuccess = (response: any) => {
  ...
  ElMessageBox.confirm(text, '导入结果', {
    ...
    cancelButtonText: '批量添加优惠券失败学员文件链接',
    ...
    .then(() => { })
    .catch((e) => {
      exportExcel(['学员'], response.data.failureResult, '批量添加优惠券失败学员ID.xlsx')
...
const downloadTemplate = async () => {
  const res = await CustomerCouponApi.getCustomerCouponTemplate()
  download.excel(res, '优惠券批量添加模版.xlsx')
}
</script>
