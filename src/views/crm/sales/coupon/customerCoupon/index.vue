<template>
  <ContentWrap>
    ...
    <el-form class="-mb-15px search-form" :model="queryParams" ref="queryFormRef" inline label-width="100">
      <el-row>
        ...
        <el-col :xl="6" :lg="8" :md="12" :sm="24" :xs="24">
          <el-form-item label="优惠券" prop="couponId" @keyup.enter="handleQuery">
            <el-select v-model="queryParams.couponId" placeholder="请选择或检索优惠券" clearable filterable multiple
              collapse-tags collapse-tags-tooltip :max-collapse-tags="2" class="!w-240px">
              <el-option v-for="option in couponOptions" :key="option.value" :label="option.label"
                :value="option.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :xl="6" :lg="8" :md="12" :sm="24" :xs="24">
          <el-form-item label="状态" prop="status" @keyup.enter="handleQuery">
            <el-select v-model="queryParams.status" placeholder="请选择状态" clearable class="!w-240px">
              <el-option v-for="dict in getStrDictOptions(DICT_TYPE.CRM_COUPON_USED_STATUS)" :key="dict.value"
        ...
        <el-col :xl="6" :lg="8" :md="12" :sm="24" :xs="24">
          <el-form-item label="添加渠道" prop="channel" @keyup.enter="handleQuery">
            <el-select v-model="queryParams.channel" placeholder="请选择添加渠道" clearable filterable multiple collapse-tags
              collapse-tags-tooltip :max-collapse-tags="3" class="!w-240px">
        ...
        <el-col :xl="6" :lg="8" :md="12" :sm="24" :xs="24">
          <el-form-item label="订单号" prop="useOrderNum" @keyup.enter="handleQuery">
            <el-input v-model="queryParams.useOrderNum" placeholder="请输入订单号" clearable class="!w-240px" />
          </el-form-item>
        </el-col>
        <el-col :xl="6" :lg="8" :md="12" :sm="24" :xs="24">
      ...
      <el-form-item>
        <el-button @click="handleQuery" v-hasPermi="['crm:sales:customerCoupon:query']">
          <Icon icon="ep:search" class="mr-5px" /> 搜索
        </el-button>
        <el-button @click="resetQuery" v-hasPermi="['crm:sales:customerCoupon:query']">
          <Icon icon="ep:refresh" class="mr-5px" /> 重置
        </el-button>
        <el-button type="primary" plain @click="openForm('create')" v-hasPermi="['crm:sales:customerCoupon:create']">
          <Icon icon="ep:plus" class="mr-5px" /> 新增
        </el-button>
        <el-button type="warning" plain @click="handleImport" v-hasPermi="['crm:sales:customerCoupon:import']">
          <Icon icon="fluent:multiselect-16-filled" class="mr-5px" /> 批量添加
        </el-button>
        <el-button type="success" plain @click="handleExport" :loading="exportLoading"
          v-hasPermi="['crm:sales:customerCoupon:export']">
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" stripe border max-height="70vh" ref="tableRef" :row-key="(row) => row.id"
      ...
      <el-table-column label="订单号" align="center" prop="useOrderNum" min-width="200">
        <template #default="scope">
          <el-button v-if="scope.row.useOrderNum" link type="primary" @click="handleCopy">
            {{ scope.row.useOrderNum }}
          </el-button>
          <span v-else>-</span>
        </template>
      </el-table-column>
      <el-table-column label="添加渠道" align="center" prop="channel" min-width="95">
        <template #default="scope">
          <arco-tag :type="DICT_TYPE.CRM_COUPON_ADD_CHANNEL" :value="scope.row.channel" />
        </template>
      </el-table-column>
      <el-table-column label="有效期" align="center">
        <el-table-column label="开始" align="center" prop="expiredFrom" :formatter="dateFormatterYMDHm" min-width="110" />
        <el-table-column label="结束" align="center" prop="expiredTo" :formatter="dateFormatterYMDHm" min-width="110" />
      </el-table-column>
      <el-table-column label="使用时间" align="center" prop="employTime" :formatter="dateFormatterYMDHm" min-width="110" />
      <el-table-column label="被推荐人" align="center" prop="referrerCustomerName" min-width="100"
        :formatter="formatEmptyValue" />
      <el-table-column label="添加人" align="center" prop="creatorName" min-width="120" :formatter="formatEmptyValue" />
      <el-table-column label="创建时间" align="center" prop="createTime" :formatter="dateFormatterYMDHm" min-width="110" />
      <el-table-column label="备注" align="center" prop="remark" show-overflow-tooltip min-width="110" />
      <el-table-column label="操作" align="center" width="130" fixed="right"
        v-hasPermi="['crm:sales:customerCoupon:update', 'crm:sales:customerCoupon:delete']">
        <template #default="scope">
  ...
  <!-- 学员优惠券 表单 -->
  <CustomerCouponForm ref="formRef" @customer-coupon-success="getList" />
  <!-- 批量导入学员优惠券 表单 -->
  <BatchImportCouponForm ref="batchImportCouponFormRef" @success="getList" />
  <!-- 学员详情 抽屉 -->
...
import {
  ...
  getAllCouponOptions, handleRemoteCustomerOptions, handleRemoteUserOptions,
} from '@/utils/dict'
...
import CustomerCouponForm from './CustomerCouponForm.vue'
import BatchImportCouponForm from './BatchImportCouponForm.vue'
import { CustomerDrawer } from '@/layout/components/CustomerDrawer'

/** 学员优惠券 列表 */
defineOptions({ name: 'CustomerCoupon' })
...
const list = ref<CustomerCouponVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await CustomerCouponApi.getCustomerCouponPage(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await CustomerCouponApi.exportCustomerCoupon(exportParams)
    download.excel(data, '学员优惠券.xlsx')
  } finally {
...
/** 批量导入按钮操作 */
const batchImportCouponFormRef = ref()
const handleImport = () => {
  batchImportCouponFormRef.value.open()
}

/** 初始化 **/
onMounted(async () => {
  deptList.value = handleTree(await DeptApi.getSimpleKuaiKuDeptList())
  couponOptions.value = await getAllCouponOptions()
  if (route.query.customerId && route.query.orderNum) {
    queryParams.customerId = route.query.customerId
    queryParams.useOrderNum = route.query.orderNum
    await fetchRemoteCustomerOptions(queryParams.customerId)
  }
  getList()
})
</script>
