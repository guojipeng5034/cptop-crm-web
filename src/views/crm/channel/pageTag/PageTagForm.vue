  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100" v-loading="formLoading">
      ...
      <el-form-item label="标签说明" prop="tagExplain">
        <el-input v-model="formData.tagExplain" placeholder="请输入标签说明" />
      </el-form-item>
      <el-form-item label="标签创建者" prop="tagUserId">
        <el-select v-model="formData.tagUserId" clearable filterable remote :loading="userOptionsLoading"
          :remote-method="fetchRemoteUserOptions" placeholder="请输入标签创建者">
...
/** 渠道页面标签 表单 */
defineOptions({ name: 'PageTagForm' })
...
const formRules = reactive({
  ...
  tagExplain: [{ required: true, message: '标签说明不能为空', trigger: 'blur' }],
  tagUserId: [{ required: true, message: '标签创建者不能为空', trigger: 'blur' }],
...
const submitForm = async () => {
  ...
  try {
    const data = formData.value as unknown as ChannelPageTagVO
    if (formType.value === 'create') {
      await ChannelPageTagApi.createChannelPageTag(data)
      message.success(t('common.createSuccess'))
    } else {
      await ChannelPageTagApi.updateChannelPageTag(data)
