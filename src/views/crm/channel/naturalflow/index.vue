<template>
    ...
    <el-form class="-mb-15px" :model="queryParams" ref="queryFormRef" inline label-width="110">
      <el-row>
        <el-col :xl="6" :lg="8" :md="12" :sm="24" :xs="24">
          <el-form-item label="流量来源" prop="naturalFlowSource" @keyup.enter="handleQuery">
            <el-select v-model="queryParams.naturalFlowSource" placeholder="请选择流量来源" clearable class="!w-240px">
        ...
        <el-col :xl="6" :lg="8" :md="12" :sm="24" :xs="24">
          <el-form-item label="流量状态" prop="status" @keyup.enter="handleQuery">
            <el-select v-model="queryParams.status" placeholder="请选择流量状态" clearable class="!w-240px">
      ...
      <el-form-item>
        <el-button @click="handleQuery" v-hasPermi="['crm:channel-natural-flow:query']">
          <Icon icon="ep:search" class="mr-5px" /> 搜索
        </el-button>
        <el-button @click="resetQuery" v-hasPermi="['crm:channel-natural-flow:query']">
          <Icon icon="ep:refresh" class="mr-5px" /> 重置
        </el-button>
        <el-button type="primary" plain @click="openForm('createChannel', 4)"
          v-hasPermi="['crm:channel-natural-flow:create']">
          <Icon icon="ep:plus" class="mr-5px" /> 新增
        </el-button>
        <el-button type="success" plain @click="handleExport" :loading="exportLoading"
          v-hasPermi="['crm:channel-natural-flow:export']">
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" stripe show-overflow-tooltip border max-height="70vh"
      :row-key="(row) => row.id">
      <el-table-column label="流量来源" align="center" prop="naturalFlowSource">
        <template #default="scope">
          <arco-tag :type="DICT_TYPE.NATURAL_FLOW_SOURCE" :value="scope.row.naturalFlowSource" />
        </template>
      </el-table-column>
      <el-table-column label="注册方式" align="center" prop="naturalFlowRegisterType">
        <template #default="scope">
          <arco-tag :type="DICT_TYPE.NATURAL_FLOW_REGISTER_TYPE" :value="scope.row.naturalFlowRegisterType" />
        </template>
      </el-table-column>
      ...
      <el-table-column label="流量状态" align="center" prop="status">
        <template #default="scope">
          <arco-tag :type="DICT_TYPE.COMMON_STATUS" :value="scope.row.status"
            v-hasNotPermi="['crm:channel-natural-flow:update']" />
      ...
      <el-table-column label="操作" align="center" width="250" fixed="right">
        <template #default="scope">
          <el-button link type="success" @click="openForm('createPage', 4,
            {
              id: scope.row.id,
          ...
          <el-button link type="primary" @click="openForm('updateChannel', 4, { id: scope.row.id })"
            v-hasPermi="['crm:channel-natural-flow:update']">
            编辑
          </el-button>
        </template>
...
/** 自然流量渠道 列表 */
defineOptions({ name: 'NaturalFlow' })

/* 配置项 */
const message = useMessage() // 消息弹窗
const loading = ref(true) // 列表的加载中
const list = ref<ChannelNaturalFlowVO[]>([]) // 列表的数据
const total = ref(0) // 
...
const handleStatusChange = async (row) => {
  try {
    ...
    await message.confirm(`确认要${text}该自然流量吗?`)
    // 发起修改状态
    await ChannelNaturalFlowApi.updateChannelNaturalFlow(row)
  } catch {
    // 取消后，进行恢复按钮
    row.status =
...
const getList = async () => {
  loading.value = true
  try {
    const data = await ChannelNaturalFlowApi.getChannelNaturalFlowPage(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await ChannelNaturalFlowApi.exportChannelNaturalFlow(queryParams)
    download.excel(data, '自然流量渠道.xlsx')
  } finally {
