...
import qs from 'qs'
import { RouterHelperUtil } from '@/utils/routerHelperUtil'
...
// 后端控制路由生成
export const generateRoute = (routes: AppCustomRouteRecordRaw[], isTop:boolean=true): AppRouteRecordRaw[] => {

  // 重处理原始菜单 只处理第一级菜单,否则会导致死循环
  if(isTop){
    try{
      routes = RouterHelperUtil.getSystemMenu(routes);
      //routes = RouterHelperUtil.processOtherModuleForRoute(routes);
    }catch(e){
      console.error('路由重处理异常,请检查菜单配置:', e);
    }
  }
  ...
  for (const route of routes) {
    ...
    if (!route.children && route.parentId == 0 && route.component) {
      ...
      if (route.children) {
        data.children = generateRoute(route.children, false)
